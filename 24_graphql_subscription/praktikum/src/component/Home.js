...
const Home = () => {
  ...
  const [EditUser, { error: updateErr, loading: updateLoading }] =
    useMutation(UpdatePessenger);

  const {
    data: subscriptionData,
    loading: loadingSubscriptionsCreated,
    error: errorSubscriptionsCreated,
  } = useSubscription(SubscribePassenger);
  ...
  if (errorSubscriptionsCreated) {
    console.log(errorSubscriptionsCreated);
    return null;
  } else if (errGetUser) {
